...
════════════════════════════ --}}
    <div class="relative px-7 pt-14 pb-0 overflow-hidden">
        ...
        <div class="flex flex-wrap items-center gap-3 mb-6" data-animate>
            ...
            <span class="text-sm text-zinc-500 dark:text-zinc-400">{{ $post['read_time'] }} min read</span>
        </div>
        ...
        >
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div class="text-sm text-zinc-500 dark:text-zinc-400">{{ $post['author']['role'] }}</div>
                </div>
            </div>
            {{-- Share --}}
            <div class="flex items-center gap-2">
                <span class="text-sm text-zinc-500 dark:text-zinc-400 mr-1">Share</span>
                @foreach ([
                ...
                ['LinkedIn',     'M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z'],
                    ...
                    <button
                        ...
                        class="w-8 h-8 rounded-lg border border-zinc-200 dark:border-zinc-800 flex items-center justify-center text-zinc-500 dark:text-zinc-400 hover:border-zinc-300 dark:hover:border-zinc-600 hover:text-zinc-700 dark:hover:text-zinc-300 transition-all duration-200"
                    >
    ...
    <div class="relative border-t border-zinc-200 dark:border-zinc-800 mt-8">
        ...
        >
            ...
            <div class="px-7 py-12">
                <div
                    class="prose prose-zinc dark:prose-invert max-w-none prose-headings:font-display prose-headings:font-bold prose-headings:tracking-tight prose-h2:text-2xl prose-h2:mt-10 prose-h2:mb-4 prose-h3:text-lg prose-h3:mt-8 prose-h3:mb-3 prose-p:text-zinc-600 dark:prose-p:text-zinc-500 dark:text-zinc-400 prose-p:leading-relaxed prose-p:text-[0.9375rem] prose-a:text-gold-800 dark:prose-a:text-gold-400 dark:prose-a:text-gold-400 prose-a:no-underline hover:prose-a:underline prose-strong:text-zinc-900 dark:prose-strong:text-white prose-blockquote:border-gold-400 prose-blockquote:bg-gold-50/50 dark:prose-blockquote:bg-gold-950/10 prose-blockquote:px-5 prose-blockquote:py-1 prose-blockquote:rounded-r-lg prose-blockquote:not-italic prose-li:text-zinc-600 dark:prose-li:text-zinc-500 dark:text-zinc-400 prose-li:text-[0.9375rem] prose-code:text-gold-800 dark:prose-code:text-gold-400 dark:prose-code:text-gold-400 prose-hr:border-zinc-200 dark:prose-hr:border-zinc-800"
                >
                    {!! $post['body'] !!}
                </div>
                    ...
                    <div class="mt-10 pt-8 border-t border-zinc-200 dark:border-zinc-800">
                        <div class="flex flex-wrap gap-2">
                            @foreach ($post['tags'] as $tag)
                                <span
                                    class="px-3 py-1.5 rounded-lg text-sm font-medium border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/40 text-zinc-500 dark:text-zinc-400"
                                >
                                    {{ $tag }}
                                </span>
                            @endforeach
                ...
                >
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <div
                                class="text-sm text-gold-800 dark:text-gold-400 dark:text-gold-400 mb-2 font-semibold uppercase tracking-wide"
                            >
                                {{ $post['author']['role'] }}
                            </div>
            ...
            <div class="px-6 py-12 space-y-8">
                ...
                {{-- CTA card --}}
                <div
                    class="p-5 rounded-xl border border-gold-200 dark:border-gold-800/40 bg-gold-50 dark:bg-gold-950/20"
                >
                    <h4 class="font-display font-bold text-sm text-zinc-900 dark:text-white mb-2">
                        Work with Rapsoltech Consulting
                    </h4>
                    <p class="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed mb-4">Need help with {{ Str::lower($post['category']) }}? Let's talk about your project.</p>
                    <a
                        href="/contact"
                        class="inline-flex items-center gap-1.5 text-sm font-semibold text-gold-800 dark:text-gold-400 dark:text-gold-400 hover:gap-2.5 transition-all duration-200"
                    >
                        Book a free call
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div class="space-y-3">
                    <div>
                        <div
                            class="text-sm font-semibold uppercase tracking-[0.15em] text-zinc-500 dark:text-zinc-400 mb-1"
                        >
                            Published
                        </div>
        ...
        <div class="relative border-t border-zinc-200 dark:border-zinc-800">
            ...
            <div class="px-7 py-12">
                <div class="flex items-center justify-between gap-4 mb-8" data-animate>
                    ...
                    <a
                        href="/blog"
                        class="text-sm font-semibold text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-colors"
                    >
                        All articles →
                    </a>
                </div>
                    ...
                    @foreach ($related as $i => $rel)
                        ...
                        >
                            ...
                            <div class="p-5 flex-1 flex flex-col">
                                ...
                                <h3
                                    class="font-display font-bold text-sm text-zinc-900 dark:text-white leading-snug flex-1 group-hover:text-gold-800 dark:group-hover:text-gold-400 dark:group-hover:text-gold-400 transition-colors duration-300"
                                >
                                    {{ $rel['title'] }}
                                </h3>
                                <div
                                    class="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800 text-sm text-zinc-500 dark:text-zinc-400"
                                >
                                    {{ $rel['date'] }}
                                </div>
                            </div>
        ...
        <div class="relative px-7 py-16 flex flex-col md:flex-row items-center justify-between gap-6" data-animate>
            <div>
                <div class="flex items-center gap-3 mb-4">
                    <div class="w-8 h-px bg-gold-400"></div>
                    <span class="text-sm font-semibold uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-400">Stay informed</span>
                </div>
                <h2 class="font-display text-2xl md:text-3xl font-black text-white">Get new articles in your inbox.</h2>
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-2">No spam. Just practical thinking from our team — twice a month.</p>
            </div>
            <form class="flex flex-col sm:flex-row gap-2 w-full md:w-auto shrink-0" action="/newsletter" method="POST">
                ...
                <input
                    ...
                    required
                    class="px-4 py-3 rounded-lg text-sm border border-zinc-700 bg-zinc-800 text-white placeholder-zinc-500 focus:outline-none focus:border-gold-400 transition-colors w-full sm:w-64"
                />
                <button
                    type="submit"
                    class="px-5 py-3 rounded-lg text-sm font-semibold font-display shrink-0 bg-gold-400 hover:bg-gold-400 text-zinc-900 transition-colors duration-200"
                >
                    Subscribe
                </button>
            </form>
