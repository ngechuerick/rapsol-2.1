            ...
            <div class="flex flex-col gap-3 lg:items-end" data-animate data-delay="3">
                ...
                <div class="flex flex-wrap gap-2 lg:justify-end">
                    @foreach (['Technology','Consulting','Finance','Industry Insights','Company News'] as $cat)
                        <a
                            href="#{{ Str::slug($cat) }}"
                            class="px-3 py-1.5 rounded-lg text-xs font-medium border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 text-zinc-500 dark:text-zinc-400 hover:border-amber-300 dark:hover:border-amber-700 hover:text-amber-700 dark:hover:text-amber-400 transition-all duration-200"
                        >
                            {{ $cat }}
                        </a>
                    @endforeach
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
                ...
                >
                    ...
                    <div class="absolute top-4 left-4">
                        <span class="px-2.5 py-1 rounded-md text-xs font-semibold bg-amber-500 text-zinc-900">
                            Featured
                        </span>
                    </div>
                </div>
                ...
                <div class="order-1 lg:order-2">
                    <div class="flex items-center gap-3 mb-5">
                        <span
                            class="px-2.5 py-1 rounded-md text-xs font-semibold uppercase tracking-wide bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400"
                        >
                            {{ $featured['category'] }}
                        </span>
                    ...
                    <h2
                        class="font-display font-black text-2xl md:text-3xl text-zinc-900 dark:text-white leading-tight mb-4 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors duration-300"
                    >
                        {{ $featured['title'] }}
                    </h2>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-xs text-zinc-400 dark:text-zinc-600">{{ $featured['date'] }}</div>
                            </div>
                        </div>
                        <span
                            class="inline-flex items-center gap-1.5 text-xs font-semibold text-amber-600 dark:text-amber-400 group-hover:gap-2.5 transition-all duration-300"
                        >
                            Read article
                ...
                >
                    ...
                    <div class="flex flex-col flex-1 p-5">
                        <div class="flex items-center gap-2 mb-3">
                            <span
                                class="px-2 py-0.5 rounded text-xs font-semibold bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400"
                            >
                                {{ $post['category'] }}
                            </span>
                            <span class="text-xs text-zinc-400 dark:text-zinc-600">{{ $post['read_time'] }} min</span>
                        </div>

                        <h3
                            class="font-display font-bold text-base text-zinc-900 dark:text-white leading-snug mb-3 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors duration-300 flex-1"
                        >
                            {{ $post['title'] }}
                        </h3>

                        <p class="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed mb-5 line-clamp-2">
                            {{ $post['excerpt'] }}
                        </p>
